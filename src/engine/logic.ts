...
export class GameEngine {
	...
	loadReplay(replayState: ReplayState) {
		...
		// Re-run the replay to reconstruct the game state
		...
		if (!replayResult.isValid) {
			console.error("Failed to load replay: ", replayResult.reason);
			this.reset(Date.now()); // Fallback to a new game if replay is invalid
			return;
		}

		this.grid = replayResult.finalGrid || new Array(GRID_SIZE * GRID_SIZE).fill(0);
		this.score = replayResult.finalScore;
		this.moves = replayState.moves.length;
		this.isGameOver = replayResult.gameOver;
		// The currentShapes and other transient state will be re-generated by refillShapes as needed.
		this.currentShapes = []; // Clear current shapes, refillShapes will be called on next turn
		this.refillShapes(); // Ensure shapes are available for the next move
	}
