...
export class FloatingTextEffect implements Effect {
	isFinished: boolean = false
	life: number = 0
	readonly DURATION = 1.0
	...
	draw(ctx: CanvasRenderingContext2D, layout: Layout) {
		ctx.save()
		const progress = this.life / this.DURATION
		const alpha = 1.0 - Math.pow(progress, 2) // Fade out quadratic
		const lift = progress * 50 // Move up 50px
