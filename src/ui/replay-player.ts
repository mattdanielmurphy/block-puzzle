import { GameMove, ReplayState } from "../engine/replay"

import { BlockClearEffect } from "./effects"
...
export class ReplayPlayer {
	...
	getCurrentScore(): number {
		return this.currentMoveIndex >= 0 && this.currentMoveIndex < this.replayState.moves.length ? this.replayState.moves[this.currentMoveIndex].scoreAfter : 0
	}
	...
	private applyMove(move: GameMove): void {
		...
		const shapeIndex = this.engine.currentShapes.findIndex((s) => s && s.id === move.shapeId)
